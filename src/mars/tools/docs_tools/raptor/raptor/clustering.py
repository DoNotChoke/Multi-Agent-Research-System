...
def perform_clustering(
    ...
    all_local_clusters: list[np.ndarray] = [np.array([]) for _ in range(len(embeddings))]
    total_clusters = 0
...
class RaptorClustering(ClusteringAlgorithm):
    ...
    def __init__(
        ...
        Args:
            ...
            verbose: Enable verbose logging.
        """
        self.reduction_dimension = reduction_dimension or settings.reduction_dimension
    ...
    def cluster_nodes(
        ...
        node_clusters = [[nodes[idx] for idx in cluster] for cluster in cluster_indices]
