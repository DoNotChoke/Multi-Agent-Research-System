...
async def check_may_autonomously_fetch_url(url: str, user_agent: str, proxy_url: str | None = None) -> None:
    ...
    processed = "\n".join(
        line for line in robots_txt.splitlines() if not line.strip().startswith("#")
    )
    parser = Protego.parse(processed)
    if not parser.can_fetch(str(url), user_agent):
        raise McpError(ErrorData(
            code=INTERNAL_ERROR,
            message=f"The sites robots.txt ({robots_url}), specifies that autonomous fetching of this page is not allowed, "
...
async def fetch_url(
    ...
    is_page_html = (
            "<html" in page_text[:100] or "text/html" in content_type or not content_type
    )
...
def slice_with_pagination(text: str, *, start_index: int, max_length: int) -> tuple[
    str, bool, Optional[int], Optional[int]]:
    ...
    if truncated:
        note = (
            f"Content truncated. Call web_fetch with start_index={next_start} "
...
async def web_server(
    ...
    async def list_tools() -> list[Tool]:
        return [
            Tool(
                ...
                inputSchema=WebSearchArgs.model_json_schema()
            ),
            Tool(
                name="web_fetch",
                description=(
                ...
                inputSchema=WebFetchArgs.model_json_schema()
            )
        ]
    ...
    async def list_prompts() -> list[Prompt]:
        return [
            Prompt(
                name="web_search",
                description="Web search for relevant sources.",
                arguments=[
                    PromptArgument(
                        name="query",
            ...
            Prompt(
                name="web_fetch",
                description="Fetch page content for a specific URL. "
                            "You should avoid crawl from pdf page like: https://www.allthingsdistributed.com/files/amazon-dynamo-sosp2007.pdf",
                arguments=[
                    PromptArgument(
                        name="url",
    ...
    async def call_tool(name: str, arguments: dict) -> list[TextContent]:
        try:
            if name == "web_search":
                args = WebSearchArgs(**(arguments or {}))
                result = web_search(args)
                return [
                    TextContent(
                        type="text",
                        text=json_text(result.model_dump(mode="json")),
                    )
                ]

            elif name == "web_fetch":
                args = WebFetchArgs(**(arguments or {}))
